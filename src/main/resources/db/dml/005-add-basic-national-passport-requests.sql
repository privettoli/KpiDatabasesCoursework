...
VALUES ('AB4235',
        (SELECT `id`
         ...
         WHERE `birth_certificate_series` = 'AB'), 1,
        (SELECT `id`
         FROM `issue_reasons`
        ...
        (SELECT `id`
         ...
         WHERE `type_id` =
               (SELECT `id`
                ...
                WHERE `name` = 'Penalty because of late obtaining')
               AND `tax` = 0.05 AND `penalty_receipt_sum` = 250.05));
...
VALUES ('AB4234',
        ...
        (SELECT `id`
         FROM `issue_reasons`
         WHERE `name` = 'Issue because of loss'),
        (SELECT `id`
         FROM `penalty_receipts`
         WHERE `type_id` =
               (SELECT `id`
                FROM `penalty_receipt_types`
                WHERE `name` = 'Penalty because of loss') AND `tax` = 0.15 AND `penalty_receipt_sum` = 949.99));
...
VALUES ('AB4236',
        (SELECT `id`
         FROM `birth_certificates`
         WHERE `birth_certificate_series` = 'CD'), 1,
        (SELECT `id`
         FROM `issue_reasons`
         WHERE `name` = 'First-time issue'));
...
VALUES ('AB4237',
        (SELECT `id`
         FROM `birth_certificates`
         WHERE `birth_certificate_series` = 'EF'), 1,
        (SELECT `id`
         FROM `issue_reasons`
         WHERE `name` = 'First-time issue'));
